...
function HemoglobinAmberTest() {
  ...
  const fetchRecords = (page, size) => {
    axios
      .post("https://javixlife.org/api/labtest/getHemoglobinAmberList", {
        pageNo: page,
        size: size,
  ...
  const setBP = (hemoglobin) => {
    return (
      <span style={{ background: "#FFBF00", padding: "4px", color: "white" }}>
      ...
      render: (_, record) => (
        <div>
          <p className="text-bold-500 mb-0">
            <span style={{ margin: "20px;", padding: "4px;" }}>
              {setBP(record.hemoglobin)}
            </span>
            &nbsp;&nbsp;
          </p>
        </div>
