...
class LipidPaneltriglyGreen extends React.Component {
...
//  }
    ...
    setBP(triglycerides){
        ...
        if(triglycerides==60 && triglycerides<=130){
            return(<span style={{background:'#008000',padding:'4px',color:'white',borderRadius:'25px;'}}>triglycerides:{triglycerides}</span>);
         }else{
            return(<span style={{background:'#008000',padding:'4px',color:'white'}}>triglycerides:{triglycerides}</span>);
         }
    }
    setSOP2(val){
        if(val>95 && val<=100 ){
      ...
      },
      {
        ...
        cell: row => (
         <div>     
          ...
          <span style={{margin:'20px;',padding:'4px;'}}>{this.setBP(row.triglycerides)}</span>&nbsp;&nbsp;
...
componentDidMount() {console.log("DID MOUNT ************");
    ...
    // if(localStorage.getItem("severity")==="1"){
		  ...
		  axios.post('http://localhost:3001/api/labtest/LipidPanelTesttriglyceridesGreenList')
		 .then(response => {
...
export default LipidPaneltriglyGreen;
