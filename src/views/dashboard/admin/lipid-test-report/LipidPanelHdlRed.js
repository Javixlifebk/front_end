...
axios.post("https://javixlife.org:3010/api/labtest/LipidPanelhdlRedList" ,{
  "pageNo":page,
  "size":size,
...
const filterData = (data) =>
    ...
    const setBP=(hdlcholesterol)=>{

      if(hdlcholesterol>=35 &&  hdlcholesterol==95 ){
        return(<span style={{background:'red',padding:'4px',color:'white'}}>hdlcholesterol:{hdlcholesterol}</span>);
     }else{
    return(<span style={{background:'red',padding:'4px',color:'white'}}>hdlcholesterol:{hdlcholesterol}</span>);
 }
  }
   const  getCitizenScreener=(_screenerId) =>{
   ...
   
    {
    ...
    
      {
        ...
        render: (_, record) => (
          ...
          <span style={{margin:'20px;',padding:'4px;'}}>{setBP(record.hdlcholesterol)}</span>&nbsp;&nbsp;
    ...
    return (
      <>
       <Row>
      <Col sm="12">
      <CardTitle><b><h3>HDL Lipid Panel Red Cases</h3></b> </CardTitle>
      </Col>          
      </Row>
...
export default  LipidPanelHdlRed