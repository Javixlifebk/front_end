  ...
  this.state = {
    columns: [
      ...
      },
      {
        ...
        sortable: true,
        cell: row => (
          <p className="text-bold-500 text-truncate mb-0">{row.ngoRegistrationNo.replace('&#x2F;','/')}</p>
        )
      },
	   {
        name: "Ngo Name",
...
componentDidMount() {
		...
		axios.post('https://javixlife.org:3010/api/ngo/ngoList', {status:1 ,ngoLoginId:localStorage.getItem("ngoId")})
		 .then(response => {
