...
const fetchRecords = (page,size) => {
  axios.post("https://javixlife.org:3010/api/labtest/getHeartRateGreenList" ,{
    "pageNo":page,
    "size":size,
...
const filterData = (data) =>
    ...
    const setBP =(hemoglobin)=>{
      ...
      if(hemoglobin<14 && hemoglobin>18){
          return(<span style={{background:'#008000',padding:'4px',color:'white',borderRadius:'25px;'}}>Heartrate:{hemoglobin}</span>);
       }else{
          return(<span style={{background:'#008000',padding:'4px',color:'white'}}>Heartrate:{hemoglobin}</span>);
       }
  }
   const  getCitizenScreener=(_screenerId) =>{
    ...
    return (
      <>
       <Row>
      <Col sm="12">
      <CardTitle><b><h3>Heart Rate Green Cases</h3></b> </CardTitle>
      </Col>          
      </Row>
