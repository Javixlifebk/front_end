...
const fetchRecords = (page,size) => {
  axios.post("https://javixlife.org:3010/api/labtest/getHeartRateRedList" ,{
    "pageNo":page,
    "size":size,
...
const filterData = (data) =>
    ...
    const setBP=(hemoglobin)=>{
      ...
      if(hemoglobin>17 &&  hemoglobin<12 ){
              return(<span style={{background:'red',padding:'4px',color:'white'}}>Heartrate:{hemoglobin}</span>);
           }else{
          return(<span style={{background:'red',padding:'4px',color:'white'}}>Heartrate:{hemoglobin}</span>);
       }
  }
   const  getCitizenScreener=(_screenerId) =>{
    ...
    return (
      <>
       <Row>
      <Col sm="12">
      <CardTitle><b><h3>Heart Rate Red Cases</h3></b> </CardTitle>
      </Col>          
      </Row>
