...
const fetchRecords = (page,size) => {
  axios.post("https://javixlife.org:3010/api/labtest/getTempratureRedList" ,{
    "pageNo":page,
    "size":size,
...
const filterData = (data) =>
    ...
    const setBP=(hemoglobin)=>{
      ...
      if(hemoglobin>17 &&  hemoglobin<12 ){
              return(<span style={{background:'red',padding:'4px',color:'white'}}>Temprature:{hemoglobin}</span>);
           }else{
          return(<span style={{background:'red',padding:'4px',color:'white'}}>Temprature:{hemoglobin}</span>);
       }
  }
   const  getCitizenScreener=(_screenerId) =>{
    ...
    return (
      <>
       <Row>
      <Col sm="12">
      <CardTitle><b><h3>Temprature Red Cases</h3></b> </CardTitle>
      </Col>          
      </Row>
