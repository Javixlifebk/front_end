...
loadRecs(recs)
 ...
 handleClick(_userid) {    
  axios.post('http://159.65.148.197:3010/api/citizen/getHistoryWomen', { citizenId:_userid})
  .then(response => {
...
componentDidMount() {
		...
		//this.setState({data:null});
		  axios.post('http://159.65.148.197:3010/api/citizen/getHistoryWomen?=', { citizenId:localStorage.getItem("citizenId")})
		 .then(response => {
					if(response.data.status===1)
					  {
						  var msg=response.data.message;
    ...
    /*axios.post('http://159.65.148.197:3010/api/report/createHistoryReport?=', { citizenId:localStorage.getItem("citizenId")})
