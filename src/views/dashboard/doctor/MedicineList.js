...
loadRecs(recs)
 ...
 handleClick(_caseId)  {    
  ...
  axios.post('http://143.244.136.145:3001/api/report/createPrescriptionReport?=', { caseId:_caseId})
  .then(response => {
...
componentDidMount() {
		...
		//this.setState({data:null});
		  axios.post('http://143.244.136.145:3001/api/doctor/prescriptionList?=', { citizenId:localStorage.getItem("citizenId")})
		 .then(response => {
					if(response.data.status===1)
					  {
						  var msg=response.data.message;
