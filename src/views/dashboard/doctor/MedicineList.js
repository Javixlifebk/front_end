...
loadRecs(recs)
 ...
 handleClick(_caseId)  {    
  ...
  axios.post('http://javixlife.org:3010/api/report/createPrescriptionReport?=', { caseId:_caseId})
  .then(response => {
...
componentDidMount() {
		...
		//this.setState({data:null});
		  axios.post('http://javixlife.org:3010/api/doctor/prescriptionList?=', { citizenId:localStorage.getItem("citizenId")})
		 .then(response => {
					if(response.data.status===1)
					  {
						  var msg=response.data.message;
