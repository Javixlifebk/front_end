...
loadRecs(recs)
 ...
 handleClick(_userid) {    
  axios.post('http://143.244.136.145:3001/api/citizen/documentList?=', {citizenId:localStorage.getItem("citizenId")})
  .then(response => {
...
componentDidMount() {
		...
		//this.setState({data:null});
		  axios.post('http://143.244.136.145:3001/api/citizen/documentsList?=', { citizenId:localStorage.getItem("citizenId"),token:'dfjkhsdfaksjfh3756237'})
		 .then(response => {
					if(response.data.status===1)
					  {
						  var msg=response.data.message;
