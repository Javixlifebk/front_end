...
componentDidMount() {
    ...
    if(localStorage.getItem("_status")==="1"){
		  ...
		  axios.post('http://159.65.148.197:3010/api/screening/getCaseDetails?=', { doctorId:localStorage.getItem("usermasid"),token:'dfjkhsdfaksjfh3756237' })
		 .then(response => {
      ...
      axios.post('http://159.65.148.197:3010/api/screening/getCaseDetails?=', { doctorId:localStorage.getItem("usermasid"),status:localStorage.getItem("_status"),token:'dfjkhsdfaksjfh3756237' })
      .then(response => {
           if(response.data.status===1)
             {
               var msg=response.data.message;
