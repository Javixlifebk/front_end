...
componentDidMount() {console.log("DID MOUNT ************");
    ...
    if(localStorage.getItem("_status")==="1"){
      ...
      axios.post('http://localhost:3001/api/screening/getCaseDetails?=', { doctorId:localStorage.getItem("usermasid"),status:localStorage.getItem("_status"),token:'dfjkhsdfaksjfh3756237' })
      .then(response => {
           console.log("Returned data:", response.data.status);
           console.log("User MasterId=" + localStorage.getItem("usermasid"))
           if(response.data.status===1)
